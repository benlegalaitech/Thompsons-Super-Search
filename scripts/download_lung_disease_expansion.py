#!/usr/bin/env python3
"""Download additional lung disease files from SharePoint.

Downloads from Closed Files from Graeme to expand the lung-disease dataset.
Targets ~1000 new unique documents.

Usage:
    python download_lung_disease_expansion.py
"""
...
# Folders to download - Closed Files from Graeme subfolders
# These contain ~1114 PDFs total
FOLDERS_TO_DOWNLOAD = [
    "Closed Files/Closed Files from Graeme/04G- Destroy 2025",      # 176 PDFs
    "Closed Files/Closed Files from Graeme/05G - Destroy 2026",     # 233 PDFs
    "Closed Files/Closed Files from Graeme/06G - Destroy 2027",     # 109 PDFs
    "Closed Files/Closed Files from Graeme/07G - Destroy 2028",     # 209 PDFs
    "Closed Files/Closed Files from Graeme/08G - Destroy 2029",     # 209 PDFs
    "Closed Files/Closed Files from Graeme/09G - destroy 2030",     # 178 PDFs
]

# Local destination - same as existing lung disease folder
LOCAL_DEST = Path(r"C:\Users\benja\LungDiseaseExpansion")

# Supported file extensions
SUPPORTED_EXTENSIONS = {'.pdf', '.doc', '.docx', '.xls', '.xlsx', '.xlsm', '.xlsb', '.msg'}
...
def get_folder_contents(folder_path: str) -> list:
    """Get all items in a SharePoint folder (non-recursive for these flat folders)."""
    items = []
...
def download_file(file_info: dict, base_folder: str) -> tuple:
    ...
    try:
        # Use folder name as subdirectory
        folder_name = os.path.basename(file_info['path'])
        local_dir = LOCAL_DEST / folder_name
...
def download_folder(folder_path: str, max_workers: int = 10):
...
def main():
    print("=" * 60)
    print("Lung Disease Dataset Expansion Download")
    print("=" * 60)
